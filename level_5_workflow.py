...
if __name__ == "__main__":
    ...
    response: Iterator[RunResponse] = workflow.run(message="Tell me a bedtime joke.")
    pprint_run_response(response, markdown=True, show_time=True)
    # Run workflow with diffrent input
    response: Iterator[RunResponse] = workflow.run(message="Tell me a meal timejoke.")
    pprint_run_response(response, markdown=True, show_time=True)
    # Run workflow again (this is immediate because of caching)
    response: Iterator[RunResponse] = workflow.run(
        message="Tell me a bedtime timejoke."
    )
    pprint_run_response(response, markdown=True, show_time=True)
